...
int IO_Data_Servo_Scan_In_Process_By_Bus( Task_Info_t *pTask, uint8_t ucBusIndex) {
	...
	for (ucIndex[ucBusIndex] = 0; ucIndex[ucBusIndex] < IO_SERVO_COUNT;ucIndex[ucBusIndex]++)
	{
		...
		if (pServo->phBus == Dynamixel_Bus_Get_By_Index(ucBusIndex))
		{
			...
			Dynamixel_Read_Data_Send(pServo->phBus, pServo->ucAdress, IO_SERVO_DATA_OFFSET, IO_SERVO_DATA_SIZE);
			do {
				CR_TASK_YELD();
				pServo = &tServoList[ucIndex[ucBusIndex]];
				iReturn = Dynamixel_Read_Data_Result(pServo->phBus,&ucError[ucBusIndex], pServo->tucDataIn);
			} while (iReturn > 0);
			if (iReturn == 0)
			{
				if( ucError[ucBusIndex] && ucError[ucBusIndex] != 0xff  )
				{
					ucIndex[ucBusIndex]--;
...
static  Task_Info_t ProcessState[IO_BUS_COUNT];
static int iBuzy = 0;

CR_TASK_INIT()

TASK_INFO_RESET(ProcessState[0]);
TASK_INFO_RESET(ProcessState[1]);
TASK_INFO_RESET(ProcessState[2]);
do {
	iBuzy = 0;
...
int IO_Data_Servo_Scan_Out_Process_By_Bus(Task_Info_t *pTask, uint8_t ucBusIndex)
{
	static uint8_t ucIndex[IO_BUS_COUNT], ucOffset[IO_BUS_COUNT],ucLen[IO_BUS_COUNT], ucError[IO_BUS_COUNT];
	tIO_Servo *pServo;
	int iReturn;
	...
	for (ucIndex[ucBusIndex] = 0; ucIndex[ucBusIndex] < IO_SERVO_COUNT;	ucIndex[ucBusIndex]++)
	{
		...
		if (pServo->phBus == Dynamixel_Bus_Get_By_Index(ucBusIndex))
		{
			...
			{
				while (pServo->tucDataIn[ucOffset[ucBusIndex]]== pServo->tucDataOut[ucOffset[ucBusIndex]]
				...
				if (ucOffset[ucBusIndex] < IO_SERVO_DATA_SIZE)
				{
					...
					Dynamixel_Write_Data_Send(pServo->phBus, pServo->ucAdress,ucOffset[ucBusIndex] + IO_SERVO_DATA_OFFSET,ucLen[ucBusIndex],&(pServo->tucDataOut[ucOffset[ucBusIndex]]));
					do {
						CR_TASK_YELD();
						pServo = &tServoList[ucIndex[ucBusIndex]];
						iReturn = Dynamixel_Read_Data_Result(pServo->phBus,&ucError[ucBusIndex], pServo->tucDataIn);
...
{
	static Task_Info_t ProcessStatus[IO_BUS_COUNT];
	static int iBuzy = 0;

	CR_TASK_INIT();
	TASK_INFO_RESET(ProcessStatus[0]);
	TASK_INFO_RESET(ProcessStatus[1]);
	TASK_INFO_RESET(ProcessStatus[2]);
	do
	{
		iBuzy = 0;
